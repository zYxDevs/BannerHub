...
#### Device-test iterations (2026-06-17)
- **iter1** (build `27713306717`, apk md5 `c05ec520…`): nickname dashboard worked; in-game pill did not show.
- **iter2** (`18447cc`, build `27714407917`, apk md5 `96c3bdd9…`): **cross-process state fix.** Activation is written in the launcher process but read in `:wine`; SharedPreferences caches one instance per file name and ignores the mode on later opens, so `BhHudInjector` opening `bh_prefs` MODE_PRIVATE first made our MODE_MULTI_PROCESS open return the stale cached instance → gate read false. Moved voice state to a plain `Properties` file in `getFilesDir()` read fresh every call (`BhVoicePrefs`). Added `BhVoice`-tagged attach/gate/addView logging.
- **iter3** (`92a8857`, build `27715450070`, apk md5 `e7866583793c8dda8be7e20279788578`): **pill visibility fix + recovery.** Device screenshot showed the HUD (DecorView) rendering in-game but no pill → a `TYPE_APPLICATION_PANEL` window does NOT render over the Wine surface on the `:wine` WineActivity. Switched the pill **and** the headless WebRTC WebView to attach to the activity **DecorView** (like `BhHudInjector`); drag updates the FrameLayout topMargin. Also: client id persisted to public `/sdcard/BannerHub/voice_id.txt` (survives uninstall/clear-data — root cause of orphaned nickname claims) with app-private fallback; dashboard **Reclaim** (force re-bind) + **Release** buttons. Worker side deployed separately (see bannerhub-api log). Manually freed `nick/the412banner` (CF R2 DELETE) after the storage-migration id-regen locked the user out. PENDING: device re-test of the pill + audio.
- **iter4** (`90cdf83…`): runtime **RECORD_AUDIO** request (mic was granted=false → WebView getUserMedia failed → browser backstop; manifest perm ≠ runtime grant). Requested on Activate/Reclaim + in-game guard. Also: **🟡 In room waiting box** shown immediately on Create/Join (a solo room has no peer so WebRTC never reaches "in-call"; box upgrades to 🟢 In call + timer on peer connect). Then decoupled **pill (pinned to edge, vertical drag) from the box (drags freely via ⠿ header)** + collapsible **opacity slider** (fades the pill only). All positions persisted.
- ✅ **iter5 — DEVICE-CONFIRMED 2026-06-17: real in-game call with a friend, two-way audio.** apk md5 `854cc5a22343307de716b3e14db04d0b` (build `27719124408`). Two bugs fixed: (1) peer showed **"Guest"** despite a nickname — worker sends roster ids then names; client rendered on ids and never re-rendered when names arrived → store raw ids + `rebuildRoster()` on ids OR names change (caveat: browser share-link joiners carry no name → still Guest). (2) **call timer reset to 0** on opacity expand/collapse — re-render built a fresh Chronometer that defaulted its base to now → anchor every render to a fixed `callBase` set once on connect.

**3.7.5 FEATURE COMPLETE.** (Latest rebuild = run `27719124408` at commit `cec2113`; branch tip `ebfd59f` is docs-only — same confirmed code.)

#### Cross-build voice (3.7.5 ↔ v6) — design investigation 2026-06-17 (NOT built; user wants this BEFORE merging)
User goal: let v6 users and 3.7.5 users share a voice session. **Verdict: it works, and the backend needs ZERO changes** — the worker voice model is already fully Steam-agnostic.
- Traced `bannerhub-api/bannerhub-worker.js` + `extension/`: the room is just a string `VOICE_ID_RE = /^[a-zA-Z0-9_-]{1,40}$/`; the shared `/voice/room` mesh page is **roster-driven** (opens one RTCPeerConnection per *every* roster member, not 2 hardcoded peers; offerer = lexicographically-smaller id → deterministic across a numeric SteamID vs a UUID, no glare). So any two clients on the same room string connect, regardless of build. Same R2 signal mailbox + CF Realtime TURN both sides.
- 3.7.5: `room` = `randomCode()` (`"bh"`+5 of `abcdefghjkmnpqrstuvwxyz23456789`, BhVoiceOverlay.java:574), `self` = persisted UUID. v6: `room` = sorted SteamID pair, `self` = SteamID.
- 3.7.5 **Join input does ZERO validation** (BhVoiceOverlay.java:299–304): trims, non-empty check, `startCall(code)`.

**"Could a 3.7.5 user join a LIVE v6 call today by typing v6's long SteamID string?"** Plumbing-compatible but **NO in practice — 3 gates:** (1) v6 never *displays* its room string (derived silently from SteamIDs) → the v6 user can't share it = practical dealbreaker; (2) the SteamID-pair **separator is unverified** — two 17-digit ids = 34 chars; `-`/`_`/none → ≤40 & valid, but `:`/`|`/`,`/etc → worker rejects every post as `invalid_id`; (3) v6's overlay may map roster ids→Steam names and not render an unknown UUID peer (audio likely still flows). No selfId collision (17-digit number vs 36-char UUID, both fit RE). `/voice/nick/*` registry is 3.7.5-dashboard-only — v6 joins code rooms without claiming a nick.
